...
object DeltasPrevious {
    ...
    fun steepestSearch(
        ...
        for (i in nodes.indices) {
            ...
            var hashmap = HashMap<Array<Pair<Int, Int>>, Double>();
            ...
            while (flag) {
                ...
                var checked = deltas.map { this.removeInvalid(it) }
                var sorted = deltas.sortedBy { it.first }
                //Iterate over the LM, skipped the valid for later moves and apply the good one
    ...
    private fun removeInvalid(move: Pair<Double, Change>): Boolean {
